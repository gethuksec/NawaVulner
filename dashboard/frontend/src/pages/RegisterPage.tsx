  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-nawa-bg px-4">
      <div className="w-full max-w-md rounded-xl border border-white/10 bg-nawa-card p-8 shadow-xl">
        <h1 className="font-mono text-xl font-semibold text-white">{t("auth.registerTitle")}</h1>
        <p className="mt-1 text-sm text-slate-400">
          {t("auth.hasAccount")}{" "}
          <Link to="/login" className="text-nawa-accent hover:underline">
            {t("auth.loginLink")}
          </Link>
        </p>
        <form className="mt-6 space-y-4" onSubmit={onSubmit}>
          ...
          <div>
            ...
            <input
              id="email"
              type="email"
              className="mt-1 w-full rounded-md border border-white/10 bg-nawa-bg px-3 py-2 text-sm text-white outline-none ring-nawa-accent focus:ring-2"
              value={email}
          ...
          >
            {loading ? t("auth.processing") : t("auth.submitRegister")}
          </button>
        </form>
